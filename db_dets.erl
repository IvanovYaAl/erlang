...
%% Server
start() ->
	case dets:open_file(my_dets, [{type, bag}, {keypos, #users.name}]) of
		{ok, my_dets} ->
			register(server, spawn_link(fun() -> loop(my_dets) end)), ok;
		{error, Reason} ->
			io:format("Can't open db: ~p~n", [Reason]),
			exit(eDetsOpen)
	end.
...
receive
	{insert, Name, Location, Company, Pid} ->
		...
		Ls = lists:append(dets:match(L, #users{name=Name, location='$2', company='_'})),
		case length(Ls) == 0 of 
			true ->
				dets:insert(L, NewUser),
				Pid ! ok;
			false ->
	...
	{where_is, Name, Pid} ->
		Loc = lists:append(dets:match(L, #users{name=Name, location='$2', company='_'})),
	...
	{located_at, Location, Pid} ->
		Ls = lists:append(dets:match(L, #users{name='$1', location=Location, company='_'})),
		case length(Ls) == 0 of
			true -> Pid ! none;
			false -> Pid ! Ls
		end,
	...
	{working_at, Company, Pid} ->
		Names = lists:append(dets:match(L, #users{name='$1', location='_', company=Company})),
		case Names == [] of
			false -> Pid ! Names;
			true -> Pid ! none
		end,
	...
	{all_names, Pid} ->
		Ls = lists:merge(dets:match(L, #users{name='$1', location='_', company='_'})),
		Pid ! Ls,
		loop(L);

	{all_locations, Pid} ->
		Ls = lists:usort(lists:append(dets:match(L, #users{name='_', location='$2', company='_'}))),
		Pid ! Ls,
		loop(L);
